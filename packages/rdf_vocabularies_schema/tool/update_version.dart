...
String _updateReadme(String content, String version) {
  ...
  final pubspecPattern = RegExp(
    r'```yaml\s*\n\s*dependencies:\s*\n\s*rdf_vocabularies_schema:\s*\^[0-9]+\.[0-9]+\.[0-9]+(?:[-+].+)?',
    multiLine: true,
  );
  final updatedPubspec = '```yaml\ndependencies:\n  rdf_vocabularies_schema: ^$cleanVersion';
  ...
  final dartPubAddPattern = RegExp(
    r'```bash\s*\n\s*dart pub add rdf_vocabularies_schema(?::.*)?',
    multiLine: true,
  );
...
String _updateDocIndex(String content, String version) {
  ...
  final dartPubAddPattern = RegExp(
    r'<code class="language-bash">dart pub add rdf_vocabularies_schema(?::[^\s<]+)?',
    multiLine: true,
  );
  const updatedDartPubAdd = '<code class="language-bash">dart pub add rdf_vocabularies_schema';
  if (dartPubAddPattern.hasMatch(result)) {
    result = result.replaceAll(dartPubAddPattern, updatedDartPubAdd);
  }
