...
`rdf_vocabularies` is a Dart build tool that transforms RDF vocabularies into type-safe Dart code. Built on top of [rdf_core](https://pub.dev/packages/rdf_core), it enables Dart developers to work with RDF data using familiar, strongly-typed patterns.
...
dev_dependencies:
  ...
  rdf_vocabularies: ^0.1.0  # The code generator
```
...
targets:
  $default:
    builders:
      rdf_vocabularies|rdf_to_dart_generator:
        enabled: true
        options:
